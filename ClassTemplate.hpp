...
#include <stdio.h>					 // Нужные заголовочные файлы указываем в *.hpp
...
private:													// В полях класса модифкаторы используют либо private, либо protected (если планируется наследование)
...
public:														// Классы в С++ по умолчанию приватные
	ClassTemplate();										// Коснтруктор по умолчанию. Он может быть без тела. Если обратиться к полям обЪекта без предварительной инициализации, то получишь мусор в переменных или словишь сегу
...
//	ClassTemplate(int _fieldInt, std::string _fieldStr): fieldInt(_fieldInt), fieldStr(_fieldStr) {}; или
	ClassTemplate(int _fieldInt, std::string _fieldStr): fieldInt(42), fieldStr("String") {}; 
	ClassTemplate(ClassTemplate const &copy);				// Конструктор копирования, нужен для того чтобы инициализировать объект другим объектом. В теле пишется присвоение всех полей с помощью геттеров и сеттеров
	~ClassTemplate();										// Деструктор по умолчанию. Деструктор указывается только один, он может быть с телом, если в классе где то выделяется динамическая память либо без него

	ClassTemplate & operator=(ClassTemplate &equal);		// Оператор присваивания для случаев когда объекты одного типа присваевается друг к другу (a = b). В теле обязательно писать проверку для случая объект равен самому себе (a = a)
	...
	int getFieldInt(void);									// методы для обращения к полям класса с модификатором private и protected для извлечения данных
	std::string getFieldStr(void);							// к каждому полю отдельно
};
